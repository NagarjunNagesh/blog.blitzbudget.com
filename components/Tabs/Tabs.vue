<template>
  ...
  { 'col-12': centered && !tabNavWrapperClasses },
    tabNavWrapperClasses]">
      <ul class="nav" role="tablist" v-bind="$attrs" :class="
      [tabTypeClass,
        ...
        <li v-for="tab in tabs" class="nav-item active" data-toggle="tab" role="tablist" aria-expanded="true"
          :key="tab.id">

          <nuxt-link data-toggle="tab" role="tablist" :to="`#${tab.id}`" @click.prevent="activateTab(tab)"
            :aria-expanded="tab.active" class="nav-link" :class="{ active: tab.active }">
